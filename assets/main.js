...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  function transition(el, setFunction, { during, start: start22, end } = {}, before2 = () => {
  }, after = () => {
  }) {
  ...
  var _removeLinkedListItem = function _removeLinkedListItem2(parent, child2, firstProp, lastProp) {
    ...
    var prev = child2._prev, next2 = child2._next;
    if (prev) {
      prev._next = next2;
  ...
  var _addToTimeline = function _addToTimeline2(timeline2, child2, position, skipChecks) {
    ...
    child2._start = _round((_isNumber(position) ? position : position || timeline2 !== _globalTimeline ? _parsePosition(timeline2, position, child2) : timeline2._time) + child2._delay);
    child2._end = _round(child2._start + (child2.totalDuration() / Math.abs(child2.timeScale()) || 0));
  ...
  var _findNextPauseTween = function _findNextPauseTween2(animation, prevTime, time) {
    ...
    if (time > prevTime) {
      ...
      while (child2 && child2._start <= time) {
        if (!child2._dur && child2.data === "isPause" && child2._start > prevTime) {
          return child2;
        }
        child2 = child2._next;
      }
  ...
  var toArray = function toArray2(value, scope2, leaveStrings) {
    return _isString(value) && !leaveStrings && (_coreInitted || !_wake()) ? _slice.call((scope2 || _doc).querySelectorAll(value), 0) : _isArray(value) ? _flatten(value, leaveStrings) : _isArrayLike(value) ? _slice.call(value, 0) : value ? [value] : [];
  };
  ...
  var snap = function snap2(snapTo, value) {
    ...
    return _conditionalReturn(value, !isArray3 ? _roundModifier(snapTo) : _isFunction(snapTo) ? function(raw2) {
      ...
      var x = parseFloat(is2D ? raw2.x : raw2), y = parseFloat(is2D ? raw2.y : 0), min2 = _bigNum, closest2 = 0, i = snapTo.length, dx, dy;
      while (i--) {
        if (is2D) {
          dx = snapTo[i].x - x;
          dy = snapTo[i].y - y;
      ...
      return is2D || closest2 === raw2 || _isNumber(raw2) ? closest2 : closest2 + getUnit(raw2);
    });
  };
  ...
  var interpolate = function interpolate2(start3, end, progress, mutate) {
    ...
    if (!func) {
      var isString3 = _isString(start3), master = {}, p, i, interpolators, l, il;
      ...
      if (!interpolators) {
        ...
        func = function func2(p2) {
          return _renderPropTweens(p2, master) || (isString3 ? start3.p : start3);
        };
      }
    }
  ...
  var _callback = function _callback2(animation, type, executeLazyFirst) {
    var v = animation.vars, callback = v[type], params, scope2;
    if (!callback) {
      return;
    }
  ...
  var _ticker = function() {
    var _getTime2 = Date.now, _lagThreshold = 500, _adjustedLag = 33, _startTime = _getTime2(), _lastUpdate = _startTime, _gap = 1e3 / 240, _nextTime = _gap, _listeners = [], _id, _req, _raf, _self, _delta, _i, _tick = function _tick2(v) {
  ...
  var Timeline = /* @__PURE__ */ function(_Animation) {
    ...
    _proto2.render = function render2(totalTime, suppressEvents, force) {
      var prevTime = this._time, tDur = this._dirty ? this.totalDuration() : this._tDur, dur = this._dur, tTime = this !== _globalTimeline && totalTime > tDur - _tinyNum && totalTime >= 0 ? tDur : totalTime < _tinyNum ? 0 : totalTime, crossingStart = this._zTime < 0 !== totalTime < 0 && (this._initted || !dur), time, child2, next2, iteration, cycleDuration, prevPaused, pauseTween, timeScale, prevStart, prevIteration, yoyo, isYoyo;
        ...
        if (time >= prevTime && totalTime >= 0) {
          ...
          while (child2) {
            ...
            if ((child2._act || adjustedTime <= child2._end) && child2._ts && pauseTween !== child2) {
              ...
              child2.render(child2._ts > 0 ? (adjustedTime - child2._start) * child2._ts : (child2._dirty ? child2.totalDuration() : child2._tDur) + (adjustedTime - child2._start) * child2._ts, suppressEvents, force);
              if (time !== this._time || !this._ts && !prevPaused) {
                pauseTween = 0;
    ...
    _proto2.totalDuration = function totalDuration(value) {
      var max2 = 0, self = this, child2 = self._last, prevStart = _bigNum, prev, start3, parent;
      if (arguments.length) {
  ...
  var _addPropTween = function _addPropTween2(target, prop, start3, end, index, targets, modifier, stringFilter, funcParam) {
  ...
  var PropTween = /* @__PURE__ */ function() {
    function PropTween3(next2, target, prop, start3, change, renderer, data2, setter, priority) {
      this.t = target;
      this.s = start3;
      this.c = change;
      this.p = prop;
  ...
  var _gsap = {
    ...
    exportRoot: function exportRoot(vars, includeDelayedCalls) {
      ...
      var tl = new Timeline(vars), child2, next2;
  ...
  var _addRotationalPropTween = function _addRotationalPropTween2(plugin2, target, property, startNum, endValue, relative) {
    var cap = 360, isString3 = _isString(endValue), endNum = parseFloat(endValue) * (isString3 && ~endValue.indexOf("rad") ? _RAD2DEG : 1), change = relative ? endNum * relative : endNum - startNum, finalValue = startNum + change + "deg", direction, pt;
    if (isString3) {
  ...
  window.product = function() {
    ...
    return {
      init() {
        const rechargeSection = document.querySelector('[id^="RechargeWidget_"]');
      ...
      setVariant(e) {
        ...
        if (variantAvailable === "true") {
          ...
          add2.innerHTML = '<span class="my-auto inline-block">Add to cart</span>';
        } else {
  ...
  window.accordion = function() {
    ...
    return {
      toggle() {
        const content = this.$el.parentElement.querySelector(".accordion-content");
  ...
  function Instafeed(options) {
    assert2(!options || typeof options === "object", "options must be an object, got " + options + " (" + typeof options + ")");
    var opts = {
      accessToken: null,
    ...
    assert2(typeof opts.target === "string" || typeof opts.target === "object", "target must be a string or DOM node, got " + opts.target + " (" + typeof opts.target + ")");
    assert2(typeof opts.accessToken === "string" || typeof opts.accessToken === "function", "accessToken must be a string or function, got " + opts.accessToken + " (" + typeof opts.accessToken + ")");
    assert2(typeof opts.accessTokenTimeout === "number", "accessTokenTimeout must be a number, got " + opts.accessTokenTimeout + " (" + typeof opts.accessTokenTimeout + ")");
    assert2(typeof opts.apiTimeout === "number", "apiTimeout must be a number, got " + opts.apiTimeout + " (" + typeof opts.apiTimeout + ")");
    assert2(typeof opts.debug === "boolean", "debug must be true or false, got " + opts.debug + " (" + typeof opts.debug + ")");
    assert2(typeof opts.mock === "boolean", "mock must be true or false, got " + opts.mock + " (" + typeof opts.mock + ")");
    assert2(typeof opts.templateBoundaries === "object" && opts.templateBoundaries.length === 2 && typeof opts.templateBoundaries[0] === "string" && typeof opts.templateBoundaries[1] === "string", "templateBoundaries must be an array of 2 strings, got " + opts.templateBoundaries + " (" + typeof opts.templateBoundaries + ")");
    assert2(!opts.template || typeof opts.template === "string", "template must null or string, got " + opts.template + " (" + typeof opts.template + ")");
    assert2(!opts.error || typeof opts.error === "function", "error must be null or function, got " + opts.error + " (" + typeof opts.error + ")");
    assert2(!opts.before || typeof opts.before === "function", "before must be null or function, got " + opts.before + " (" + typeof opts.before + ")");
    assert2(!opts.after || typeof opts.after === "function", "after must be null or function, got " + opts.after + " (" + typeof opts.after + ")");
    assert2(!opts.success || typeof opts.success === "function", "success must be null or function, got " + opts.success + " (" + typeof opts.success + ")");
    assert2(!opts.filter || typeof opts.filter === "function", "filter must be null or function, got " + opts.filter + " (" + typeof opts.filter + ")");
    assert2(!opts.transform || typeof opts.transform === "function", "transform must be null or function, got " + opts.transform + " (" + typeof opts.transform + ")");
    assert2(!opts.sort || typeof opts.sort === "function", "sort must be null or function, got " + opts.sort + " (" + typeof opts.sort + ")");
    assert2(!opts.render || typeof opts.render === "function", "render must be null or function, got " + opts.render + " (" + typeof opts.render + ")");
    assert2(!opts.limit || typeof opts.limit === "number", "limit must be null or number, got " + opts.limit + " (" + typeof opts.limit + ")");
    assert2(!opts.apiLimit || typeof opts.apiLimit === "number", "apiLimit must null or number, got " + opts.apiLimit + " (" + typeof opts.apiLimit + ")");
    this._state = state;
    this._options = opts;
  }
  ...
  Instafeed.prototype._showNext = function showNext(callback) {
    ...
    scope2._debug("showNext", "pool", scope2._state.pool.length, scope2._state.pool);
    if (scope2._state.pool.length > 0) {
      ...
      scope2._debug("showNext", "updated pool", scope2._state.pool.length, scope2._state.pool);
      ...
      if (scope2._state.paging && typeof scope2._state.paging.next === "string") {
        url = scope2._state.paging.next;
      } else {
        url = "https://graph.instagram.com/me/media?fields=caption,id,media_type,media_url,permalink,thumbnail_url,timestamp,username&access_token=" + scope2._state.token;
        if (!scope2._options.apiLimit && typeof scope2._options.limit === "number") {
          scope2._debug("showNext", "no apiLimit set, falling back to limit", scope2._options.apiLimit, scope2._options.limit);
          url = url + "&limit=" + scope2._options.limit;
        } else if (typeof scope2._options.apiLimit === "number") {
          scope2._debug("showNext", "apiLimit set, overriding limit", scope2._options.apiLimit, scope2._options.limit);
      ...
      scope2._makeApiRequest(url, function onResponseReceived(err, data2) {
        ...
        try {
          ...
          if (processed.unused && processed.unused.length > 0) {
            scope2._debug("onResponseReceived", "saving unused to pool", processed.unused.length, processed.unused);
  ...
  Instafeed.prototype._processData = function processData(data2) {
    ...
    if (typeof data2 !== "object" || typeof data2.data !== "object" || data2.data.length <= 0) {
      return null;
    }
  ...
  Instafeed.prototype._valueForKeyPath = function valueForKeyPath(keyPath, data2) {
    var exp = /([\w]+)/gm;
    var match = null;
    var key = null;
    var lastValue = data2;
    ...
    apiRequest.onload = function apiRequestOnLoad(event) {
      ...
      if (contentType.indexOf("application/json") >= 0) {
        try {
          ...
          scope2._debug("apiRequestOnLoad", "json parsing error", err, apiRequest.responseText);
    ...
    if (typeof this._options.accessToken === "function") {
      ...
      try {
        this._options.accessToken(function accessTokenReceiver(err, value) {
          scope2._debug("getAccessToken", "received accessToken callback", called, err, value);
    ...
    return {
      init() {
        var feed = new instafeed_es_default({
          accessToken: "IGQWROWFhaMFExUUZA4VXYtbkt1SnFVR2gyVENySWxzQk8tSEtOYnFrNXdSS0JDRlVSbk9hRlRCYkVLOHFtLXY2Uno0aTA1NENaa0JCWmR1U2toZAm1hSmt4eEhwcFNVRms1S2VpTEhrbW4wTk1fUGVxX3lHUHJVcG8ZD",
          limit: 4,
          template: '<a aria-label="instagram post" class="block w-full" href="{{link}}"><img class="w-full" title="{{caption}}" src="{{image}}" alt="{{caption}}" loading="lazy" /></a>'
        });
        feed.run();
      }
    };
  ...
  var _getMaxScroll = function _getMaxScroll2(element, axis) {
    var dim = axis === "x" ? "Width" : "Height", scroll2 = "scroll" + dim, client = "client" + dim;
    return Math.max(0, _isRoot(element) ? Math.max(_docElement3[scroll2], _body2[scroll2]) - (_win4["inner" + dim] || _docElement3[client] || _body2[client]) : element[scroll2] - element[client]);
  };
  ...
  var _getElementBounds = function _getElementBounds2(element, context) {
    ...
    var isSVG = element.getBBox && element.ownerSVGElement, doc = element.ownerDocument || _doc4, left, right, top, bottom, matrix, p1, p2, p3, p4, bbox, width, height, cs, contextParent;
    if (element === _win4) {
  ...
  var _isClickable = function _isClickable2(element) {
    var data2;
    return !element || !element.getAttribute || element === _body2 ? false : (data2 = element.getAttribute("data-clickable")) === "true" || data2 !== "false" && (element.onclick || _clickableTagExp.test(element.nodeName + "") || element.getAttribute("contentEditable") === "true") ? true : _isClickable2(element.parentNode);
  };
  ...
  var _initCore3 = function _initCore4(required) {
    if (_windowExists5() && document.body) {
      ...
      _addPaddingBR = function() {
        ...
        div.style.cssText = child2.innerHTML = "width:90px;height:40px;padding:10px;overflow:auto;visibility:hidden";
    ...
    function Draggable2(target, vars) {
      ...
      var type = (vars.type || "x,y").toLowerCase(), xyMode = ~type.indexOf("x") || ~type.indexOf("y"), rotationMode = type.indexOf("rotation") !== -1, xProp = rotationMode ? "rotation" : xyMode ? "x" : "left", yProp = xyMode ? "y" : "top", allowX = !!(~type.indexOf("x") || ~type.indexOf("left") || type === "scroll"), allowY = !!(~type.indexOf("y") || ~type.indexOf("top") || type === "scroll"), minimumMovement = vars.minimumMovement || 2, self = _assertThisInitialized2(_this2), triggers = _toArray(vars.trigger || vars.handle || target), killProps = {}, dragEndTime = 0, checkAutoScrollBounds = false, autoScrollMarginTop = vars.autoScrollMarginTop || 40, autoScrollMarginRight = vars.autoScrollMarginRight || 40, autoScrollMarginBottom = vars.autoScrollMarginBottom || 40, autoScrollMarginLeft = vars.autoScrollMarginLeft || 40, isClickable = vars.clickableTest || _isClickable, clickTime = 0, gsCache = target._gsap || gsap2.core.getCache(target), isFixed = _isFixed3(target), getPropAsNum = function getPropAsNum2(property, unit2) {
        ...
        if (self.autoScroll && self.isDragging && (checkAutoScrollBounds || dirty)) {
          var e = target, autoScrollFactor = self.autoScroll * 15, parent, isRoot2, rect2, pointerX, pointerY, changeX, changeY, gap;
        ...
        if (_isFunction3(snap3)) {
          return function(n) {
            ...
            return snap3.call(self, n > max2 ? max2 + (n - max2) * edgeTolerance : n < min2 ? min2 + (n - min2) * edgeTolerance : n) * factor;
          };
        }
        if (_isArray2(snap3)) {
          return function(n) {
        ...
        if (shiftStart && self.isPressed && !matrix.equals(start3 || new Matrix2D())) {
      ...
      _this2.enable = function(type2) {
        ...
        if (type2 !== "soft") {
          ...
          while (--i > -1) {
            ...
            vars.allowContextMenu || _addListener(trigger2, "contextmenu", onContextMenu);
          }
          _setSelectable(triggers, false);
        }
        _addScrollListener(target, updateScroll);
  ...
  var VelocityTracker = /* @__PURE__ */ function() {
    ...
    _proto.remove = function remove2(property) {
      var pt = this._props[property], prev, next2;
      if (pt) {
        prev = pt._prev;
        next2 = pt._next;
        if (prev) {
  ...
  var _getClosest = function _getClosest2(n, values, max2, min2, radius) {
    var i = values.length, closest2 = 0, absDif = _bigNum4, val, dif, p, dist;
    if (_isObject5(n)) {
      while (i--) {
        val = values[i];
        dif = 0;
  ...
  var _parseEnd = function _parseEnd2(curProp, end, max2, min2, name, radius) {
    ...
    if (_isObject5(end)) {
      adjustedEnd = end.calculated ? end : (_isFunction5(endVar) ? endVar(end) : _getClosest(end, endVar, max2, min2, radius)) || end;
      if (!end.calculated) {
      ...
      adjustedEnd = _isFunction5(endVar) ? endVar(end) : _isArray3(endVar) ? _getClosest(end, endVar, max2, min2, radius) : parseFloat(endVar);
    }
    if (adjustedEnd > max2) {
      adjustedEnd = max2;
  ...
  var _calculateDuration = function _calculateDuration2(start3, end, velocity) {
    return Math.abs((end - start3) * _checkPointRatio / velocity / _checkPoint);
  };
  var _reservedProps2 = {
    resistance: 1,
    checkpoint: 1,
  ...
  function horizontalLoop(items, config3) {
    ...
    let onChange = config3.onChange, lastIndex = 0, tl = gsapWithCSS.timeline({
      ...
      let i = values.length, closest2 = 1e10, index = 0, d;
      while (i--) {
    ...
    if (config3.draggable && typeof Draggable === "function") {
      ...
      let wrap3 = gsapWithCSS.utils.wrap(0, 1), ratio, startProgress, draggable, dragSnap, lastSnap, initChangeX, wasPlaying, align = () => tl.progress(
  ...
  window.logos = function() {
    ...
    return {
      init() {
        const logos = gsapWithCSS.utils.toArray(this.$el.querySelectorAll(".logo"));
